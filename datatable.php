...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.Course','s.contact','s.joindate');
	...
	/* DB table to use */
	$sTable = " student as s,Course as b ";
	...
	$sWhere = " WHERE b.id=s.Course and s.delete_status='0' and s.balance>0 ";
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.Course and s.delete_status='0' and s.balance>0  ";
...
if($_GET['type']=="report")
{
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.Course','s.contact','s.joindate','s.item');
	...
	$sWhere = " WHERE b.id=s.Course and s.delete_status='0'  ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.Course and s.delete_status='0'   ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
