...
$sid = (isset($_POST['student']))?mysqli_real_escape_string($conn,$_POST['student']):'';
 ...
 $sql = "select s.id,s.sname,s.snumber,s.balance,s.fees,s.contact,b.Course,s.joindate,s.item from student as s,Course as b where b.id=s.Course and  s.delete_status='0' and s.id='".$sid."'";
$q = $conn->query($sql);
if($q->num_rows>0)
{
...
echo '  <form class="form-horizontal" id ="signupForm1" action="fees.php" method="post">
  ...
  <div class="form-group">
  <label class="control-label col-sm-2" for="email">Student Number:</label>
  <div class="col-sm-10">
    <input type="text" class="form-control" disabled  value="'.$res['snumber'].'" >
  </div>
</div>
  ...
  <div class="form-group">
    <label class="control-label col-sm-2" for="item">Purchase:</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" name="item"  id="item" value="'.$res['item'].'" disabled/>
	  <input type="hidden" value="'.$res['item'].'" name="item">
    </div>
  </div>
...
if($fq->num_rows>0)
{
 ...
 $sql = "select s.id,s.sname,s.snumber,s.balance,s.fees,s.contact,b.Course,s.joindate,s.item from student as s,Course as b where b.id=s.Course  and s.id='".$sid."'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
